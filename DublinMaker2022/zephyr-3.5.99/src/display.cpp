...
void BacklightHigh()
{
	...
	pwm_set_cycles(blk_pwm, 0, Period,Period,0);

}
void BacklightLow()
{
	uint32_t Period = (uint32_t)16000000/ (uint32_t)1000;
	pwm_set_cycles(blk_pwm, 0, Period,0,0);
}
void display::setBrightness(uint32_t pcnt)
{
	...
	*Prescaler = 6;
	...
	pwm_set_cycles(blk_pwm, 0, Period,HighTime,0);
}
...
{
	this->openAperture(x, y, x + width - 1, y + height - 1);
	struct spi_buf tx_buf = {.buf = Image, .len = (unsigned int)width*(unsigned int)height*2};
	struct spi_buf_set tx_bufs = {.buffers = &tx_buf, .count = 1};   
	DCHigh();
	spi_write(spi_display, &cfg, &tx_bufs);
